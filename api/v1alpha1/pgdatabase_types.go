...
type PgDatabaseSpec struct {
	...
	// foo is an example field of PgDatabase. Edit pgdatabase_types.go to remove/update
	// +optional
...
type PgDatabaseStatus struct {
	...
	// conditions represent the current state of the PgDatabase resource.
...
type PgDatabase struct {
	...
	// spec defines the desired state of PgDatabase
	// +required
	Spec PgDatabaseSpec `json:"spec"`

	// status defines the observed state of PgDatabase
	// +optional
	Status PgDatabaseStatus `json:"status,omitzero"`
}
...
func init() {
	SchemeBuilder.Register(func(s *runtime.Scheme) error {
		s.AddKnownTypes(SchemeGroupVersion, &PgDatabase{}, &PgDatabaseList{})
		return nil
	})
}
